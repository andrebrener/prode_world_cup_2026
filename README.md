...
- **Leaderboard** rows are clickable — tap a player to open a drawer with *all* their predictions. In Fun pools it also shows the **pure total** (points without cards) next to the card-adjusted total — and the **🃏 column** measures the card swing against that Puro (so passive boosters like the buffs count too).
- **Match-day panel** shows the day's fixtures with everyone's predictions side by side, plus a **🔮 simulator**: punch in hypothetical scores and watch the projected table shift before the games are played. In Fun pools each row breaks the points into **match / 🃏 card / 🔥 streak**, and a steal shows up as a **🥩 +X to Player** chip on the thief's row. Matches are ordered by their real UTC kickoff instant.
- **Attack modal.** When you aim a hostile card, the target picker lists each player with their **current table position and points**, so you know who you're hitting.
...
- **Point swap: 🔄 Game is game.** A legendary, blockable attack (`frozen_swap`): you pick someone and **swap totals** with them — you end up with their points, they end up with yours. It's variable because the amount is the gap between you two **at that instant** (`D = their total − yours`, frozen there, one-shot). Swap with someone above you and you climb while they drop — but you *can* target someone below (it'd backfire), so choose wisely. A shield voids it; a mirror **inverts** it back onto you.
...
- **Secret defenses with a decoy.** When you play a shield/mirror, the pool sees a **decoy** — a plausible positive, self-targeting card (deterministic per play) instead of the real defense, so attackers can't tell you're protected. The truth is revealed at **midnight (CDMX)**; the owner sees their own decoy with a private note. If you draw an attack and don't aim it at anyone, it **bounces back on you** (autotiro).
- **Auto-curse.** Players who never claim their card for the day still get an **auto-maldición** applied — hiding doesn't shield you from the day's curses.
...
- **The engine (code).** A closed set of reusable, parameterized **outcomes** — the *math* of a card — lives in [`src/lib/cards.ts`](src/lib/cards.ts): `multiply_match` (×2 / ×3 / ÷2 over the first match of the day, a chosen match, or the whole day), `flat_points`, `steal_day_points`, `var_bonus`, `zero_day`, `shield`, `upstream_forecast`, `social_overlay`, `champion_points`, `frozen_penalty` (a one-shot penalty computed against the table at play time — powers Caparazón Azul), `frozen_delta` (snap to your Puro — powers Baño de realidad), `frozen_swap` (one-shot swap of two players' totals, frozen at play time — powers Game is game), and more. Every card maps to one of these, so re-skinning a card **never** changes the scoring — the math always comes from here.
...
- **Draw config, per pool.** `pool_fun_config` holds the odds: the **rarity weights** — common / rare / legendary / curse (default `50 / 25 / 10 / 15`). The draw is a single level by rarity (social/ego cards are just common cards — no separate "no-effect" tier). A fifth **"Extra"** tier (weight `0`) sits outside the rarity draw entirely — it holds the **positional cards** (below), which fall through their own per-position gate, so adding or removing extras never shifts the other rarities' odds.
...
- **Karma de cartas (part of the same toggle).** On top of the position bias, the draw is **also** biased by **how much you inflated your score with the cards** — the *timba*, measured as **Total − Puro** (card-adjusted total minus pure points) relative to the group. Whoever gained the most from cards leans toward more curses / fewer legendaries; whoever the cards hurt leans the other way. This rides on top of the table bias (it doesn't replace it): the player who climbed by luck eats more curse than the one who climbed by a good eye (high Puro, low luck). Both axes only apply when Karma is on.
- **Editing.** Owners/admins manage the deck and draw config from the pool's admin screen (`/p/[slug]/admin`), with cards grouped by rarity: rename / re-emoji / re-rarity / enable each card, add or remove cards (pick a mechanic), tune the per-rarity draw weights, and toggle Karma. For attack mechanics, the admin can also pin a **fixed target** (*blanco fijo*) — whoever draws the card is forced to throw it at that one person ("a card with a name on it") — both on existing deck cards and when adding a new one.
